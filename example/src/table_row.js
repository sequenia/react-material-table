...
class TableRow extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return JSON.stringify(this.state.columns) !== JSON.stringify(nextState.columns) ||
  ...
  cellContent(column) {
    ...
    if(column.type === "enum") {
      component = <EnumCell column = { column } 
                            value = { value }
                            onChange = { this.onChangeColumn } />
    }
    if(column.type === "text") {
      component = <TextCell column = { column } 
                            value = { value }
                            onChange = { this.onChangeColumn } />
    }
    if(column.type === "dateTime") {
      component = <DateTimeCell column = { column}
                                value = { value }
                                onChange = { this.onChangeColumn } />
    }
    if(column.type === "model") {
      component = <ModelCell column = { column }
                             value = { value }
                             onChange = { this.onChangeColumn } />
    }
    if(column.type === "boolean") {
      component = <BooleanCell column = { column }
                               value = { value }
                               onChange = { this.onChangeColumn } />
    }
    if(itemLink) {
      component = <Link to = { itemLink }
  ...
  render() {
    ...
    return <MaterialTableRow className = { clsx({
      [classes.root]: true, 
      ...
      {
        columns.map((column) => {
          let content = this.cellContent(column);
